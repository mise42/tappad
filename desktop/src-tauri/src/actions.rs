...
fn capability_for_action(action: &str) -> CapabilityStatus {
    ...
    #[cfg(target_os = "windows")]
    {
        match action {
            ...
            "nightlight.toggle" => capability(
                "downgraded",
                Some("Windows beta opens Night light settings instead of toggling immediately."),
            ),
            _ => capability("supported", None),
        }
    }
...
fn linux_command(action: &str) -> Option<&'static str> {
    match action {
        ...
        "screenrecord.screen" => Some(
            r#"mkdir -p "$HOME/Videos/TapPad"; gpu-screen-recorder -w "$(hyprctl monitors -j | jq -r '.[] | select(.focused == true) | .name')" -k auto -f 60 -fm cfr -fallback-cpu-encoding yes -o "$HOME/Videos/TapPad/screenrecording-$(date +'%Y-%m-%d_%H-%M-%S').mp4" & pkill -RTMIN+8 waybar"#,
        ),
        "screenrecord.window" => Some(
            r#"mkdir -p "$HOME/Videos/TapPad"; region=$(hyprctl activewindow -j | jq -r '"\(.at[0]),\(.at[1]) \(.size[0])x\(.size[1])"'); gpu-screen-recorder -w "$region" -k auto -f 60 -fm cfr -fallback-cpu-encoding yes -o "$HOME/Videos/TapPad/screenrecording-$(date +'%Y-%m-%d_%H-%M-%S').mp4" & pkill -RTMIN+8 waybar"#,
        ),
        "screenrecord.screen.audio" => Some(
            r#"mkdir -p "$HOME/Videos/TapPad"; gpu-screen-recorder -w "$(hyprctl monitors -j | jq -r '.[] | select(.focused == true) | .name')" -k auto -f 60 -fm cfr -fallback-cpu-encoding yes -a "default_output|default_input" -ac aac -o "$HOME/Videos/TapPad/screenrecording-$(date +'%Y-%m-%d_%H-%M-%S').mp4" & pkill -RTMIN+8 waybar"#,
        ),
        "screenrecord.screen.webcam" => Some(
            r#"mkdir -p "$HOME/Videos/TapPad"; device=$(v4l2-ctl --list-devices 2>/dev/null | grep -m1 "^\s*/dev/video" | tr -d '\t'); [[ -z $device ]] && notify-send "No webcam found" -u critical -t 3000 && exit 1; busy=$(lsof "$device" 2>/dev/null | tail -n +2 | awk '{print $1}' | head -1); [[ -n $busy ]] && notify-send "Webcam in use by $busy" -u critical -t 5000 && exit 1; scale=$(hyprctl monitors -j | jq -r '.[] | select(.focused == true) | .scale'); target=$(awk "BEGIN {printf \"%.0f\", 360 * $scale}"); ffplay -f v4l2 -framerate 30 "$device" -vf "crop=iw/2:ih,scale=${target}:-1" -window_title "WebcamOverlay" -noborder -fflags nobuffer -flags low_delay -probesize 32 -analyzeduration 0 -loglevel quiet & sleep 2; gpu-screen-recorder -w "$(hyprctl monitors -j | jq -r '.[] | select(.focused == true) | .name')" -k auto -f 60 -fm cfr -fallback-cpu-encoding yes -o "$HOME/Videos/TapPad/screenrecording-$(date +'%Y-%m-%d_%H-%M-%S').mp4" & pkill -RTMIN+8 waybar"#,
        ),
        ...
        "nightlight.toggle" => Some("omarchy toggle nightlight"),
