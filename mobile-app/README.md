...
The app browses `_tappad._tcp.local`. A discovered host must still pass TapPad's token authorization before the credential is saved in the platform secure store. Scan the selected Desktop Host's pairing QR code, or enter its token manually as a fallback. After a successful pairing, the app reconnects to that host automatically and loads the host-served control UI.
